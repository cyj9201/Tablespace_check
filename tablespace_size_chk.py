    ...
    for i in range(len(datafile_all)):
        for j in range(len(datafile_all[i])):
            if datafile_all[i]['MAXBYTES'][j] != 10000:
                scale_up.append("ALTER DATABASE DATAFILE '{}' AUTOETEND ON NEXT 1000M MAXSIZE 10000M".format(datafile_all[i]['FILE_NAME'][j]))
        if len(datafile_all[i][datafile_all[i].MAXBYTES != 10000]) == 0:
            scale_out.append("ALTER TABLESPACE {} ADD DATAFILE '{}' SIZE 1M AUTOEXTEND ON NEXT 1000M MAXSIZE 10000M".format(
                                all_tablespace_result[i]['TBS_NM'],datafile_all[i]['FILE_NAME'][len(datafile_all[i])-1]))
